...
```python
pip download -d "dir_to_download_files" --only-binary=:all: --implementation cp --proxy "proxy:port" "pkg_name"
```
for which I also have been using in combination of `--cert` specifier but I discarded it after keep getting OSError fail as described above.
